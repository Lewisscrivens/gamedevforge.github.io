...
- <code>Name</code>: Java-Hello-World - (The name of your project)
- <code>Location</code>: C:\Users\YOUR_USER_NAME\Documents
- <code>Language</code>: Java
- <code>JDK</code>: Oracle OpenJDK version 22
...
Once these are filled out, make sure to uncheck <code>Generate Code with onboarding tips</code> and then press Create. The new project should open in IDEA as seen below.
...
This is fine for running our <code>Main.java</code> file and printing "Hello World!" to the console, but it can be confusing when working across multiple files without understanding
...
In the context of a simple Java "Hello World!" program, the application's entry point is the <code>Main</code> method.
...
- The <code>System.out.println</code> statement is used to print "Hello World!" to the console.
...
The default run/debug configuration in your IDEA project should be <code>Current File</code>. As we continue we will be creating multiple Java files making it a pain to use.
So we are going to create a new run/debug configuration that will always run the Entry point in the <code>Main.java</code> file. Follow the steps below to set this up.

Along the top of the IDEA window, select the active configuration drop-down (If not yet changes it will read <code>Current File</code>).\
Select <code>Edit Configurations...</code>
...
Give this configuration a name, in this case, we are setting up a run/debug configuration for running our main entry point in the <code>Main.java</code> file.
I have named it <code>Run Application</code> as it indicates what this configuration is for. Make sure to check <code>Store as project file</code>,
...
<code>Main</code> as this is the file we wish to build and run in this configuration.
...
The newly created <code>Run Application</code> configuration as default from the drop-down menu.\
Let's give it a test by pressing the green play button.

![Edit Configuration Test](./img/IDEA_Configuration_Test.webp)

The <code>Run Application</code> configuration should have executed the Main entry point,
...
Changing the input to <code>System.out.println</code> to anything else, for example I will change it to <code>My first Java Project!</code>, and running the application again will now print the new message.
