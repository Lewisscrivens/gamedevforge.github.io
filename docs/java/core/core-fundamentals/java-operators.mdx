...
Take the expression `5 + 3` for instance, `5` and `3` are the operands and `+` is the operator in this scenario.
...
These are mainly used in control-flow structures such as `if`, `while`, and `for` loops, where they help determine the
...
I don't want to go into too much detail here as there are already resources available for explaining operator precedence:
