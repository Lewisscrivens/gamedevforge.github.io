...
this based off what is actively being stored in them. There are many different primitive types supported by Java, they represent
single values and are directly supported.

This differs from a Java Object which is an instance of a <TextTooltip text="Class"/>. Objects enable the implementation of concepts like <TextTooltip text="Abstraction"/>,
<TextTooltip text="Inheritance"/>, and <TextTooltip text="Polymorphism"/>, which we will explore in more detail in the [Object Orientation Page](/docs/java-object-orientation).
...
In total there are eight primitive types in Java:
...
An important thing to understand before moving on to the explanations of primitive data types is bits. Think of a bit as the smallest unit of information in a computer.
...
This is not only the case for memory and storage but also for computation, the way computers perform calculations, make decisions, and execute instructions is
done by manipulating bits in various ways.

## Boolean

The boolean type can have one of two values, <code>true</code> and <code>false</code>. It is used for evaluating logical conditions, it is typically used
for comparisons or control flow statements, such as if statements, while loops, for loops etc. all of which we will later go over in the [Control Flow Page](/docs/java-control-flow).
...
<code>Typically occupies 1 bit but is not explicitly defined</code>
...
conservation is required, dealing with raw binary data, or when there is a very small range of values expected. If treated as a <TextTooltip text="unsigned"/> integer it can be
interpreted as 0 to 255 but that will be discussed in more detail in later pages.

When a variable makes use of the char byte, you can initialize it with an integer value within the range of -128 to 127.
If you choose not to initialize a byte variable, it will automatically be assigned a default value of 0.
If you choose to initialize a byte variable outside of the -128 to 127 range will result in a compilation error.
...
conservation is required or when dealing with data that falls within a moderate range of values. If treated as a <TextTooltip text="unsigned"/> integer it can be
interpreted as 0 to 65,535 but that will be discussed in more detail in later pages.

When using a variable of type short, you can initialize it with an integer value within the range of -32,768 to 32,767.
If you choose not to initialize a short variable, it will automatically be assigned a default value of 0.
Attempting to initialize a short variable outside of the specified range will result in a compilation error.
...
These types are commonly used for general-purpose integer-based calculations and when dealing with data that requires a wider range of values. Java doesn't directly
support <TextTooltip text="unsigned"/> numerical types, to treat it as an unsigned integer you would need to use a long type to represent the range as 0 to 4,294,967,295.

When using a variable of type int, you can initialize it with an integer value within the range of -2,147,483,648 to 2,147,483,647.
If you choose not to initialize an int variable, it will automatically be assigned a default value of 0.
Attempting to initialize an int variable outside of the specified range will result in a compilation error.
...
When using a variable of type long, you can initialize it with an integer value within the range of -9,223,372,036,854,775,808 to 9,223,372,036,854,775,807.
If you choose not to initialize a long variable, it will automatically be assigned a default value of 0. 
Attempting to initialize a long variable outside of the specified range will result in a compilation error.
...
so for game calculations that are approximated, they are widely used.

To expand on what a floating point is, take the number -1.2651111. We can split this up to the following:

- 1 bit is used to represent the sign <code>-</code>.
- 8 bits for the exponent <code>1</code>.
- 23 bits for the mantissa <code>2651111</code>.
...
A float has limited precision and can only represent a certain number of decimal digits.
As the exponent increases, the bits required to represent the value increases (Think about it, a byte has only 8 bits and can only represent a value between -128 to 127).
...
So storing 12557.2651111 would result in a float value of 12557.265.
...
To expand on what a floating point is, let's consider the number -1.26511111212331. We can split this up as follows:

- 1 bit is used to represent the sign <code>-</code>.
- 11 bits for the exponent <code>1</code>.
- 52 bits for the mantissa <code>-</code>.

I won't go in to much more detail than this, the concept is the same as the float but instead of single precision we have double precision allowing larger values in both the
exponent and matissa parts of the number.
